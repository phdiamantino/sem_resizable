...
d3v4.queue()
    ...
    .await(function (error, t, w) {
    const worker = new Worker(window.URL.createObjectURL(new Blob([t], {
      ...
      console.log(d_extent_color)
        ...
        circles.on("click", clicked)
            ...
            function clicked(d, i) {
               ...
               showTableColNames() 
                console.log(d)
                const d_row_filter = [d_row.filename.substr(0, d_row.filename.length - 5),
                                      d_row.reference,
                                      d_row.patern.substr(0, 5),
                                      d_row.heuristic.substr(0, 4),
                                      d_row.score.substr(0, 4), 
                                      d_row.count_debts.substr(0, 1),
                                      d_row.r,
                                      d_row.index]; 
...
d3v4.csv("data/excomment_mp_apache.csv", function (data) {  
    data = data.slice(70,503)
    console.log(data)
          ...
          var circles = svgMP.selectAll('circle')
              ...
              // Funções TSNE
                ...
                .force('tsne', function(alpha){
                  data.forEach((d,i) => {
                    // itera sobre POS e define as novas posições dos pontos
                    d.x += alpha * (1500*pos[i][0] - d.x);
                    d.y += alpha * (1450*pos[i][1] - d.y);
                  });
                })
                .force('orientation', function(){
                  let tx = 0,
                ...
                .on('tick', function () {
                  //Posição da Vis
                  circles
                    .attr('cx', d => 220 + xScale(d.x*s - d.y*c))
                    .attr('cy', d => 200 + xScale(d.x*c + d.y*s))
                    //.attr("class", "non_brushed")
                  circles.append('g')
                  .append('title')
                      ...
                      .call(brush);
                       ...
                       function update(){
                        d3v4.selectAll(".form-check-input").each(function(d){
                          ...
                          // Se estiver marcado, a força é aplicada
                          if(cb.property("checked")){
                            forcetsne.force('collide', d3v4.forceCollide().radius(d => 30 + d.r))
                          // Se estiver desmarcado, atua sem a força
                          }else{
                            forcetsne.force('collide', d3v4.forceCollide().radius(d => 1))
                          }
                        })
                      }
                      // Quando o botão muda, executa a atualização
